...
struct ZoomURLPatternTests {
    ...
    @Test func testGovernmentVsStandardDistinction() {
        ...
        let standardInStandard = standardPattern.regex!.matches(in: standardURL, options: [], range: standardRange)
        let standardInGov = govPattern.regex!.matches(in: standardURL, options: [], range: standardRange)
    ...
    @Test func testVanityVsStandardDistinction() {
        ...
        let standardRange = NSRange(location: 0, length: standardURL.utf16.count)
        let standardInStandard = standardPattern.regex!.matches(in: standardURL, options: [], range: standardRange)
        let standardInVanity = vanityPattern.regex!.matches(in: standardURL, options: [], range: standardRange)
        
        #expect(standardInStandard.count > 0, "Standard URL should match standard pattern")
        #expect(standardInVanity.count == 0, "Standard URL should NOT match vanity pattern (no subdomain)")
        
        // Test vanity URL
        let vanityRange = NSRange(location: 0, length: vanityURL.utf16.count)
        let vanityInStandard = standardPattern.regex!.matches(in: vanityURL, options: [], range: vanityRange)
        let vanityInVanity = vanityPattern.regex!.matches(in: vanityURL, options: [], range: vanityRange)
        
        #expect(vanityInStandard.count > 0, "Vanity URL should match standard pattern (*.zoom.us includes zoom.us)")
